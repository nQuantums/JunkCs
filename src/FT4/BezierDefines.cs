...
namespace FT4 {
	[DataContract]
	public class BezierDefines : CodeGen {
		[DataMember]
		public BezierDefine[] defines;
		...
		public static BezierDefines FromJsonFile(string path) {
			var s = File.ReadAllText(path, Encoding.UTF8);
			using (var ms = new MemoryStream(Encoding.UTF8.GetBytes(s))) {
				var serializer = new DataContractJsonSerializer(typeof(BezierDefines));
				return serializer.ReadObject(ms) as BezierDefines;
			}
		}
		...
		/// <param name="genProc">Bezier定義とT4テンプレートから<see cref="generationEnvironment"/>にソースを生成するデリゲート</param>
		public void Generate(string outputDir, StringBuilder generationEnvironment, Action<BezierDefine> genProc) {
	...
	[DataContract]
	public class BezierDefine : CodeGen {
		[DataMember]
		public string type;
		[DataMember]
		public int length;
		...
		public BezierDefine[] OtherTypes {
			get {
		...
		public string ClassName {
			get {
				return "Bezier" + this.length + this.Postfix;
			}
		}
		...
		public BezierDefine Clone(TypeDefine type) {
			var c = this.MemberwiseClone() as BezierDefine;
			c.type = type.Name;
			c._TypeDefine = type;
			return c;
		}
