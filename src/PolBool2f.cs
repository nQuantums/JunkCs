//#define POLBOOL_GLOBALLOG_FRAME // GlobalLogger のフレームでログ出力を行う
//#define POLBOOL_GLOBALLOG_FRAME_COMMENT // GlobalLogger のフレーム内でのさらに詳細なログ出力を行う
...
namespace Jk {
	...
	public class PolBool2f : IJsonable {
		...
		public struct PolygonIndices : ICollection<int>, IJsonable {
			const uint Mask0 = 0x7fffu;
			const uint Mask1 = 0x7fffu << 15;
			const uint Mask = Mask0 | Mask1;
            const uint One0 = 1;
            const uint One1 = 1 << 15;

            /// <summary>
            /// 全要素をパックした値、ビット事に違う意味を割り当てている
            /// <para>0...15: ０グループ目ポリゴンインデックス</para>
            /// <para>16...29: １グループ目ポリゴンインデックス</para>
            /// <para>30...31: 全グループ数</para>
            /// </summary>
            uint _Value;
			...
			/// <summary>
			/// 有効なポリゴンインデックスが設定されているインデックスの最大値
			/// </summary>
			public int IndexMax {
				get {
					unchecked {
                        ...
                        // ↓のビット演算では↑と同じ結果になる処理をしている
                        var v = this._Value;

                        // 負数に1加算して立つビットを一旦最上位へシフトしてから逆にシフトすることでマスクを作成する
                        // ポリゴンインデックスが-1の場合 0xffffffff、それ以外は 0x00000000 になる
                        var invalidMask0 = (uint)(((int)(((v & Mask0) + One0) << 16)) >> 31);
                        var invalidMask1 = (uint)(((int)(((v & Mask1) + One1) << 1)) >> 31);

                        // 全グループのインデックスが-1の場合のみ-1を
                        // グループ０のインデックスが-1以外の場合には0
                        // グループ１のインデックスが-1以外の場合には1となる
                        return (int)((0xffffffffu & invalidMask0 & invalidMask1) | (~invalidMask1 & 1));
                    }
                }
			}
			...
			public PolygonIndices(int length) {
				this._Value = Mask | ((uint)length << 30);
			}
		...
		public class Edge : IGridSpaceItem2f, IJsonable {
			...
			/// </summary>
			public int RightGroupMax {
				get {
					return this.RightPolygons.IndexMax;
				}
			}
			...
			/// </summary>
			public int LeftGroupMax {
				get {
					return this.LeftPolygons.IndexMax;
				}
			}
			...
			public string OutlineString {
				get {
					return string.Concat(this.From.UniqueIndex, ": ", this.From.Position, " => ", this.To.UniqueIndex, ": ", this.To.Position, ", " , this.LeftPolygons.ToJsonString(), " | ", this.RightPolygons.ToJsonString());
				}
			}
			...
			public void SetUserData(bool right, int groupIndex, object userData) {
				if (right) {
					ref var d = ref this.RightUserData;
					if (d == null)
						d = new object[this.RightPolygons.Length];
					d[groupIndex] = userData;
				} else {
					ref var d = ref this.LeftUserData;
					if (d == null)
						d = new object[this.LeftPolygons.Length];
					d[groupIndex] = userData;
				}
			}
			...
			/// <summary>
			/// このエッジから見た指定ノードのインデックスを取得する
			/// </summary>
			/// <param name="node">ノード</param>
			/// <returns>-1:リンクされていない、0:<see cref="From"/>にリンクされている、1:<see cref="To"/>にリンクされている</returns>
			...
			/// <summary>
			/// エッジの途中へのノード挿入情報を登録する
			/// </summary>
			/// <param name="t">位置を指定するパラメータ</param>
			...
			/// <summary>
			/// 指定されたグループの属性を左右入れ替える
			/// </summary>
			/// <param name="groupIndex">グループインデックス</param>
			public void Swap(int groupIndex) {
				// ポリゴンインデックスの入れ替え
				var polygonIndex = this.RightPolygons[groupIndex];
				this.RightPolygons[groupIndex] = this.LeftPolygons[groupIndex];
				this.LeftPolygons[groupIndex] = polygonIndex;

				// ユーザーデータの入れ替え
				ref var rd = ref this.RightUserData;
				ref var ld = ref this.LeftUserData;
				if (rd == null && ld == null)
					return; // 両側にデータが存在しないなら何もする必要はない

				if (rd != null && ld != null) {
					// 両側に存在するなら普通に入れ替え
					var d = rd[groupIndex];
					rd[groupIndex] = ld[groupIndex];
					ld[groupIndex] = d;
				} else if (rd != null) {
					// 右側から左へ移動
					ld = new object[rd.Length];
					ld[groupIndex] = rd[groupIndex];
					rd[groupIndex] = null;
				} else {
					// 左から右へ移動
					rd = new object[ld.Length];
					rd[groupIndex] = ld[groupIndex];
					ld[groupIndex] = null;
				}
			}
			...
			/// <summary>
			/// 指定されたグループの属性を左右でコピーする
			/// </summary>
			/// <param name="toRight">左側から右側へコピーするかどうか</param>
			/// <param name="groupIndex">グループインデックス</param>
			public void Copy(bool toRight, int groupIndex) {
				ref var rd = ref this.RightUserData;
				ref var ld = ref this.LeftUserData;
				if (toRight) {
					this.RightPolygons[groupIndex] = this.LeftPolygons[groupIndex];
					...
					this.LeftPolygons[groupIndex] = this.RightPolygons[groupIndex];
					if (rd == null && ld == null)
						return; // 両側にデータが存在しないなら何もする必要はない
					if (rd != null && ld != null) {
						ld[groupIndex] = rd[groupIndex];
			...
			public override string ToString() {
				if (InRecursiveCall(this, this.From, this.To)) {
					return Jsonable.Fields(nameof(this.UniqueIndex), this.UniqueIndex, nameof(this.OutlineString), this.OutlineString, nameof(this.Flags), this.Flags, nameof(this.RightPolygons), this.RightPolygons, nameof(this.LeftPolygons), this.LeftPolygons, nameof(this.UserValue), this.UserValue);
				} else {
					try {
						EnterRecursiveCall(this);
						return Jsonable.Fields(nameof(this.UniqueIndex), this.UniqueIndex, nameof(this.OutlineString), this.OutlineString, nameof(this.Flags), this.Flags, nameof(this.From), this.From, nameof(this.To), this.To, nameof(this.RightPolygons), this.RightPolygons, nameof(this.LeftPolygons), this.LeftPolygons, nameof(this.UserData), this.UserData, nameof(this.UserValue), this.UserValue);
					} finally {
						LeaveRecursiveCall(this);
					}
				}
		...
		public struct EDir : IJsonable {
			...
			public string OutlineString {
				get {
					if (this.Edge == null)
						return null;
					return string.Concat(this.Edge.OutlineString, ", ", this.TraceRight ? "right" : "left");
				}
			}

			public override string ToString() {
				return Jsonable.Fields(nameof(this.TraceRight), this.TraceRight, nameof(this.Edge), this.Edge, nameof(this.OutlineString), this.OutlineString);
			}
		...
		public void CreateTopology(bool validation = false) {
#if POLBOOL_GLOBALLOG_FRAME
...
#if POLBOOL_GLOBALLOG_FRAME
			using (GlobalLogger.NewFrame(new GlobalLogger.FrameArgs(
				nameof(rightEnabledFlag), rightEnabledFlag,
				nameof(leftEnabledFlag), leftEnabledFlag)))
#endif
			{
				var edges = new FList<EDir>();
				var polygons = new FList<FList<EDir>>();
				GetPolygons(edges, edgeFilter, false, EdgeFlags.RightRemoved, EdgeFlags.LeftRemoved, rightEnabledFlag, leftEnabledFlag, polygons);
		...
		/// <param name="traceMinAngle">辿るエッジを探す際に左回りでも最小角度になるものがあればそれを辿るなら true</param>
		...
		private void GetPolygons(FList<EDir> edges, EdgeFilter edgeFilter, bool traceMinAngle, EdgeFlags rightFlag, EdgeFlags leftFlag, EdgeFlags rightEnabledFlag, EdgeFlags leftEnabledFlag, FList<FList<EDir>> resultPolygons) {
#if POLBOOL_GLOBALLOG_FRAME
			using (GlobalLogger.NewFrame(new GlobalLogger.FrameArgs(
				nameof(traceMinAngle), traceMinAngle,
				nameof(rightFlag), rightFlag,
				nameof(leftFlag), leftFlag,
				nameof(rightEnabledFlag), rightEnabledFlag,
				nameof(leftEnabledFlag), leftEnabledFlag)))
#endif
			{
...
#if POLBOOL_GLOBALLOG_FRAME_COMMENT
				GlobalLogger.FrameCommentFields(nameof(this.Edges), new FList<Edge>(this.Edges));
#endif
				...
				foreach (var edge in this.Edges) {
					if ((edge.Flags & rightFlag) == 0) {
						...
						// 結果のポリゴン一覧に追加
						if (TracePolygon(edge, true, traceMinAngle, rightFlag, leftFlag, rightEnabledFlag, leftEnabledFlag, edges))
				...
				foreach (var edge in this.Edges) {
					if ((edge.Flags & leftFlag) == 0) {
						// ポリゴンを構成するエッジと方向一覧を取得
						// 結果のポリゴン一覧に追加
						if (TracePolygon(edge, false, traceMinAngle, rightFlag, leftFlag, rightEnabledFlag, leftEnabledFlag, edges))
							resultPolygons.Add(new FList<EDir>(edges));
					}
				}
...
#if POLBOOL_GLOBALLOG_FRAME_COMMENT
				GlobalLogger.FrameCommentFields(nameof(this.Edges), new FList<Edge>(this.Edges));
				GlobalLogger.FrameCommentFields(nameof(resultPolygons), resultPolygons);
#endif
			}
		}
		...
		private static bool TracePolygon(Edge edge, bool traceRight, bool traceMinAngle, EdgeFlags rightFlag, EdgeFlags leftFlag, EdgeFlags rightEnabledFlag, EdgeFlags leftEnabledFlag, FList<EDir> resultEdges) {
#if POLBOOL_GLOBALLOG_FRAME
			using (GlobalLogger.NewFrame(new GlobalLogger.FrameArgs(
				nameof(edge), edge.OutlineString,
				nameof(traceRight), traceRight,
				nameof(traceMinAngle), traceMinAngle,
				nameof(rightFlag), rightFlag,
				nameof(leftFlag), leftFlag,
				nameof(rightEnabledFlag), rightEnabledFlag,
...
#endif
			{
				var edir = new EDir(edge, traceRight);
				var startEdge = edir.Edge;
				var startNode = edir.From; // ポリゴンの開始ノード
				var nextNode = edir.To;
				var curNode = startNode;
				var isNull = true;
				var rightFlagAndGotFlag = rightFlag;
				var leftFlagAndGotFlag = leftFlag;
				var enabledFlags = rightEnabledFlag | leftEnabledFlag;
				var vec1v = new vector();
				var ed = new EDir();
				...
				while (true) {
					...
					edir.Edge.Flags |= edir.TraceRight ? rightFlagAndGotFlag : leftFlagAndGotFlag;
					resultEdges.Add(edir);

#if POLBOOL_GLOBALLOG_FRAME_COMMENT
					using (GlobalLogger.NewFrame(new GlobalLogger.FrameArgs(nameof(edir.OutlineString), edir.OutlineString), "BaseLoop"))
#endif
					{
						// 指定側に１つでもポリゴンが存在すればポリゴンを形成できる　※自己交差しない場合に限る
						if (isNull && (edir.TraceRight ? edir.Edge.RightPolygons.Exists : edir.Edge.LeftPolygons.Exists))
							isNull = false;

						// エッジのベクトルを計算
						var nextNodePos = nextNode.Position;
						var vec1 = curNode.Position - nextNodePos;
						var vecLen1 = edir.Edge.Length;

						// 角度判定の内積のための仕込みを行う
						if (!traceMinAngle)
							vec1v = vec1.RightAngle();

						// 次のエッジを探す
						// 指定方向のノードに接続されたエッジで且つエッジ同士のなす右回りの角度が最も小さいものを選ぶ
						// ※エッジの進行方向から見て最も右に折れ曲がるエッジを探す
						...
						for (int i = nextEdgesCore.Count - 1; i != -1; i--) {
							ed.Edge = nextEdgesCore.Items[i];
							if (ed.Edge == edir.Edge)
								continue; // 元のエッジならスキップする

							ed.TraceRight = ed.Edge.From == nextNode;

#if POLBOOL_GLOBALLOG_FRAME_COMMENT
							using (GlobalLogger.NewFrame(new GlobalLogger.FrameArgs(nameof(ed.OutlineString), ed.OutlineString), "SearchNext"))
#endif
							{
								if (ed.Edge != startEdge) {
									if ((ed.Edge.Flags & (ed.TraceRight ? rightFlag : leftFlag)) != 0) {
										continue; // 既に辿ったエッジはスキップする
									}
								}

								// 内積によりエッジ同士がなす角度の大きさを判定
								var n2node = ed.To; // さらに次のノード
								var vec2 = n2node.Position - nextNodePos;
								var vecLen2 = ed.Edge.Length;
								var cos = vec1.Dot(vec2) / (vecLen1 * vecLen2);

								if (!traceMinAngle) {
									// 右周り最小角度限定の場合、エッジ進行方向に対して左右どちらかにより符号を調整する
									if (0 <= vec1v.Dot(vec2)) {
										cos += 1;
									} else {
										cos = -1 - cos;
...
#if POLBOOL_GLOBALLOG_FRAME_COMMENT
								GlobalLogger.FrameCommentFields(nameof(cos), cos);
#endif
								...
								if (maxCos < cos) {
									nextEdge = ed.Edge;
									nextNextNode = n2node;
									nextIsRight = ed.TraceRight;
									maxCos = cos;
								}
							}
						}

						edir = new EDir(nextEdge, nextIsRight);

#if POLBOOL_GLOBALLOG_FRAME_COMMENT
						GlobalLogger.FrameCommentFields(nameof(edir.OutlineString), edir.OutlineString);
#endif
		...
		private void RebuildTpols() {
...
#endif
			{
				...
				// エッジとノードに分解されたデータから元グループ＆ポリゴンインデックス別のポリゴンを見つけ出す
...
#if INTERSECT_SELF
						GetPolygons(edges, edgeFilter, true, EdgeFlags.RightRemoved, EdgeFlags.LeftRemoved, 0, 0, tmpEPolygonEdges);
#else
						GetPolygons(edges, edgeFilter, false, EdgeFlags.RightRemoved, EdgeFlags.LeftRemoved, 0, 0, tmpEPolygonEdges);
#endif
...
#if INTERSECT_SELF
                            // エッジの進行方向右側に指定ポリゴンが存在しない（自己交差しているポリゴンはその状態になっている）なら
                            // 反対側から属性をコピーする
                            var eloopsCore = epolygon.Loops.Core;
							for (int ieloop = eloopsCore.Count - 1; ieloop != -1; ieloop--) {
								var edgesCore = eloopsCore.Items[ieloop].Edges.Core;
								for (int iedge = edgesCore.Count - 1; iedge != -1; iedge--) {
									var ed = edgesCore.Items[iedge];
									if ((ed.TraceRight ? ed.Edge.RightPolygons : ed.Edge.LeftPolygons)[groupIndex] < 0)
                                        ed.Edge.Copy(ed.TraceRight, groupIndex); // コピーすることで自己交差をOR
                                    //ed.Edge.Swap(groupIndex); // 入れ替えることで自己交差をXORの様な感じになる
								}
							}
#endif
						}
		...
		public override string ToString() {
			if (InRecursiveCall(this)) {
				...
				try {
					EnterRecursiveCall(this);
					return Jsonable.Fields(nameof(this.Groups), this.Groups, nameof(this.TopoGroups), this.TopoGroups, nameof(this.Nodes), new FList<Node>(this.Nodes), nameof(this.Edges), new FList<Edge>(this.Edges));
				} finally {
					LeaveRecursiveCall(this);
				}
			}
