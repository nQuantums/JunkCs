...
using vector = Jk.Vector4d;

namespace Jk {
	[XmlType("Jk.Vector4d")]
	[StructLayout(LayoutKind.Explicit, Pack = 8, Size = 32)]
	[Serializable]
	public struct Vector4d : IJsonable {
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public Vector4d(element x, element y, element z, element w) {
			X = x;
			Y = y;
			Z = z;
			W = w;
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public Vector4d(UnityEngine.Vector4 v) {
			X = (element)v.x;
			Y = (element)v.y;
			Z = (element)v.z;
			W = (element)v.w;
