...
namespace Jk {
	...
	[Serializable]
	public struct Vector4f : IJsonable {
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public Vector4f(element x, element y, element z, element w) {
			X = x;
			Y = y;
			Z = z;
			W = w;
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public vector Floor() {
			return new vector((element)Math.Floor(X), (element)Math.Floor(Y), (element)Math.Floor(Z), (element)Math.Floor(W));
		}

		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public vector Ceil() {
			return new vector((element)Math.Ceiling(X), (element)Math.Ceiling(Y), (element)Math.Ceiling(Z), (element)Math.Ceiling(W));
		}
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public Vector4f(UnityEngine.Vector4 v) {
			X = (element)v.x;
			Y = (element)v.y;
			Z = (element)v.z;
			W = (element)v.w;
